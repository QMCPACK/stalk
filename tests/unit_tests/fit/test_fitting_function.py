...
def test_FittingFunction():
    ...
    grid.add_point(10.0)
    fit = FittingFunction(minimize_pf, {'pfn': 2})
    ...
    fit_noisy = fit.find_minimum(grid, Gs=Gs)
    ...
    assert match_to_tol(fit_noisy.x0_err, 0.0)
    assert match_to_tol(fit_noisy.y0_err, 0.0)

    # Test getting distribution (TODO: incomprehensive in statistical sense!)
    N = 10
    x0s, y0s = fit.get_distribution(*grid.valid_args, N=N)
    assert len(x0s) == N
    assert len(y0s) == N
    x0s2 = fit.get_x0_distribution(*grid.valid_args, N=N)
    y0s2 = fit.get_y0_distribution(*grid.valid_args, N=N)
    assert len(x0s2) == N
    assert len(y0s2) == N

    # Test __eq__
    assert fit == FittingFunction(minimize_pf, {'pfn': 2})
    assert fit != []
